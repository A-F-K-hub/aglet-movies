...
const HomeScreen = () => {
  ...
  const fetchData = async (searchKeyWord) => {
    const request = instance.get(
      `/search/movie?api_key=${API_KEY}&language=en-US&query=${searchKeyWord}&page=1&include_adult=false`
    );
    return request;
  };
  ...
  const addMovieToFavorites = async (_id) => {
    await appNetworkService
      .put(`users/${currentUser._id}`, {
        favourites: [...usersFavoriteMovies, _id],
      })
      .then(() => {
        ...
        {moviesResults.map((movie) => {
          return (
            (movie.poster_path || movie.backdrop_path) && (
              <div>
                <img
                  className={`carousel-poster ${"carousel-posterLarge"}`}
                  key={movie.id}
